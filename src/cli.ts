...
import { getAvailablePlayers, PLAYER_DEFINITIONS, PLAYER_NAMES, playAudioStream } from './players'
import { DEFAULT_VOICE_ID, ttsStream } from './services/elevenlabs'

const CLI_VERSION = '1.0.0'

function formatModelsForHelp() {
  const orderedModels = [DEFAULT_MODEL, ...TTS_MODELS.filter((model) => model !== DEFAULT_MODEL)]
...
function printPlayers() {
  ...
  for (const player of availablePlayers) {
    ...
    console.log(`  ${COLOR_MODEL(player.name)} ${COLOR_META(`(${player.description})`)} - ${status}`)
  }
}
...
export async function runCli(argv: string[]) {
  ...
  cli
    ...
    .action(async (text: string[] = [], options: Record<string, unknown>) => {
      ...
      logger.debug(`input: model=${model} voice=${voice} player=${player ?? 'auto'} text_length=${joinedText.length}`)
