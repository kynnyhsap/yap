...
export function resolvePlayer(explicitPlayer?: string) {
  if (explicitPlayer) {
    if (!PLAYER_NAMES.includes(explicitPlayer as PlayerName)) {
      throw new Error(`Unsupported player: ${explicitPlayer}. Run --players to see supported players.`)
    }
    ...
    if (!Bun.which(selectedDefinition.command)) {
      throw new Error(`Player ${selectedName} is not installed. Run --players to inspect availability.`)
    }
  ...
  throw new Error('No supported audio player found. Install mpv, ffplay, cvlc, mplayer, or mpg123.')
}
...
export async function playAudioStream(
  ...
  options.logger.debug(`player: selected=${player.name} command=${player.command} args=${player.args.join(' ')}`)
  ...
  options.logger.debug(`stream: finished total=${totalMs.toFixed(1)}ms bytes=${bytes} exit=${exitCode}`)
